...
#
#   NORMA_PROJECT=<id> ./tooling/scripts/harness-cycle.sh
#   DRY_RUN=1 NORMA_PROJECT=<id> ./tooling/scripts/harness-cycle.sh   # no writes
#
# Env (or an operator env file at $NORMA_ENV_FILE):
#   NORMA_PROJECT   (required)  tracker project/epic id
#   NORMA_CONFIG    (optional)  path to a config JSON (default: software-dev preset)
#   NORMA_SLUG      (optional)  epic slug for durable context
...
ENV_FILE="${NORMA_ENV_FILE:-$HOME/.config/norma/env}"
if [[ -f "$ENV_FILE" ]]; then
...
: "${NORMA_PROJECT:?set NORMA_PROJECT to the tracker project/epic id}"

LOG_DIR="$ROOT/.norma/logs"; mkdir -p "$LOG_DIR"
STAMP="$(date +%Y-%m-%d_%H%M%S)"
LOG_FILE="$LOG_DIR/cycle_$STAMP.log"

LOCK_DIR="$ROOT/.norma/.cycle.lock"
...
ARGS=(cycle "$NORMA_PROJECT")
[[ -n "${NORMA_CONFIG:-}" ]] && ARGS=(--config "$NORMA_CONFIG" "${ARGS[@]}")
[[ -n "${NORMA_SLUG:-}" ]] && ARGS+=(--slug "$NORMA_SLUG")
